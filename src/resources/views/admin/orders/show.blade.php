...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-2">Детали заказа и связанные билеты</p>
        </div>
        ...
        <div class="bg-white shadow-md rounded-lg p-6 max-w-2xl">
            ...
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Билеты в заказе</h3>
                <div class="bg-gray-50 p-4 rounded-lg">
                    @forelse($order->orderItems as $item)
                        <div class="mb-2">
                            <p class="text-sm text-gray-600">
                                <strong>{{ $item->ticket->number }}</strong> ({{ $item->ticket->voyage->name ?? 'N/A' }}, {{ number_format($item->ticket->price, 2, ',', ' ') }} ₽)
                            </p>
                        </div>
                    @empty
