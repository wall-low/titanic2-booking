...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Редактировать место ({{ $place->type === 'departure' ? 'Отправления' : 'Прибытия' }})</h1>
            <p class="text-gray-600 mt-2">Обновите данные места</p>
        </div>
            ...
            <form action="{{ route('admin.places.update', $place) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <select name="type" id="type"
                            ...
                            required>
                        <option value="departure" {{ old('type', $place->type) === 'departure' ? 'selected' : '' }}>Отправление</option>
                        <option value="arrival" {{ old('type', $place->type) === 'arrival' ? 'selected' : '' }}>Прибытие</option>
                    </select>
                    @error('type')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex gap-3">
                    ...
                    <a
                        href="{{ route('admin.places.index', ['type' => $place->type]) }}"
