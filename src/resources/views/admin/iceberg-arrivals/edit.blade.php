...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white shadow-md rounded-lg p-6 max-w-2xl">
        <form action="{{ route('admin.iceberg-arrivals.update', $iceberg) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6 p-4 bg-gray-50 rounded-lg">
                <p class="text-sm text-gray-600">
                    <strong>ID:</strong> {{ $iceberg->id }}
                </p>
                <p class="text-sm text-gray-600">
                    <strong>Создано:</strong> {{ $iceberg->created_at->format('d.m.Y H:i') }}
                </p>
                <p class="text-sm text-gray-600">
                    <strong>Обновлено:</strong> {{ $iceberg->updated_at->format('d.m.Y H:i') }}
                </p>
            </div>
