...
<div class="container-fluid px-0">
    ...
    <section class="hero-section text-center py-5" style="
        background: linear-gradient(rgba(15,23,42,0.8), rgba(30,41,59,0.8)), url('/images/i.webp');
        ...
        <div>
            <h1 class="display-4 fw-bold mb-3" style="font-family: Georgia, serif;">Путешествия на айсберги</h1>
            <p class="lead mb-0 text-light">Незабываемые приключения среди ледяных просторов</p>
        </div>
    </section>
    ...
    <div class="container py-5" style="background: #0f172a;">
                    ...
                    <div class="col-md-6 col-lg-4">
                        <div class="card flight-card h-100 border-0 shadow-lg" style="background: #1e293b;">
                            <div class="card-header text-center py-3" style="background: linear-gradient(135deg, #1e293b, #334155); border-bottom: 2px solid #fbbf24;">
                                ...
                                <small style="color: #fcd34d;">{{ $voyage->placeDeparture->name }} → {{ $voyage->icebergArrival->name }}</small>
                            </div>
                                ...
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <p class="mb-0 fw-bold">{{ \Carbon\Carbon::parse($voyage->departure_date)->format('d.m.Y H:i') }}</p>
                                        </div>
                                        <div>
                                            <small>Прибытие</small>
                                            <p class="mb-0 fw-bold">{{ \Carbon\Carbon::parse($voyage->arrival_date)->format('d.m.Y H:i') }}</p>
                                        </div>
                                    </div>
                                </div>
                                <p class="fw-semibold mb-1">Длительность: {{ $voyage->travel_time }} ч</p>
                                <h4 class="fw-bold text-warning">{{ number_format($voyage->base_price, 0) }} ₽</h4>
                            </div>
                            <div class="card-footer bg-transparent text-center" style="border-top: 1px solid #334155;">
                                <a href="{{ route('shop.voyage', $voyage->id) }}" class="btn btn-gold btn-sm fw-bold">ВЫБРАТЬ РЕЙС</a>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        @else
            <div class="text-center text-muted py-5">Рейсов пока нет. Скоро появятся!</div>
        @endif

        <!-- Развлечения -->
        @if($entertainments->count() > 0)
            <h2 class="text-center mt-5 mb-4 fw-bold" style="color: #fbbf24;">РАЗВЛЕЧЕНИЯ НА БОРТУ</h2>
            <div class="row g-4">
                @foreach($entertainments as $entertainment)
                    <div class="col-md-6 col-lg-3">
                        <div class="card text-center border-0 shadow-sm" style="background: #1e293b; color: #fcd34d; border: 1px solid #334155;">
                            <div class="card-body">
                                <h5 class="fw-bold">{{ $entertainment->name }}</h5>
                                <p class="display-6 fw-bold text-warning">{{ number_format($entertainment->price, 0) }} ₽</p>
